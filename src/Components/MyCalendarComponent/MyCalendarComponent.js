...
const MyCalendarComponent = () => {
  ...
  const generateDays = () => {
    ...
    for (let i = 0; i < firstDayOfMonth; i++) {
      days.push(<div key={`empty-${i}`} className="day empty"><div className="innerDay"></div></div>);
    }
    for (let i = 1; i <= daysInMonth; i++) {
      ...
      days.push(
        ...
        >
          ...
          {i}
          {specialDays[dateKey] && <div className="special">{specialDays[dateKey]}</div>}
          </div>
        </div>
      );
    }
  ...
  return (
    <div className="mainCalendar">
      <div className="header2">
        <p className="monthYear">{monthNames[month]} {year}</p>
